...
class EventKey extends CombinedKey<FestivalId, EventId> {
  const EventKey({required FestivalId festivalId, required EventId eventId})
...
typedef MyScheduleProvider =
    StateNotifierProviderFamily<
      MyScheduleController,
      AsyncValue<MySchedule>,
      FestivalId
    >;
...
class MyScheduleController extends StateNotifier<AsyncValue<MySchedule>> {
  ...
  Future<Optional<MySchedule>> _readFromAppStorage(String fileName) {
  ...
  Future<MySchedule> _readFromLegacyFile() {
    ...
    return _readFromAppStorage(
      Constants.myScheduleAppStorageLegacyFileName,
    ).then(
      (result) => result
          .map((mySchedule) {
  ...
  Future<void> _loadMySchedule() async {
    final mySchedule = await _readFromAppStorage(_appStorageFileName).then(
      (result) => result.orElseGetAsync(
        handleLegacyFile ? _readFromLegacyFile : () async => MySchedule.empty(),
      ),
    );
...
class MyScheduleProviderCreator {
  ...
  static LikedEventProvider createLikedEventProvider() => Provider.family(
    (ref, eventKey) => ref
        .watch(dimeGet<MyScheduleProvider>()(eventKey.festivalId))
        .when(
